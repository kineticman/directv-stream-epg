...
def main() -> int:
    ...
    username = os.getenv("DTV_USERNAME", "")
    ...
    def on_request(request):
        ...
        try:
            url = request.url
            if TARGET_SUBSTRING in url and not captured_auth:
                ...
                print(f"[CAPTURED] Auth request: {url[:80]}...")
        except:
            pass
    ...
    with sync_playwright() as p:
        ...
        while time.time() < timeout_at:
            if captured_auth and captured_tokens:
                break
            time.sleep(0.5)
